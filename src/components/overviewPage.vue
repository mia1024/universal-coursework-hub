    ...
    <el-container>
      ...
      <el-main style="">
        ...
        <el-row style="padding-top: 2.5em" id="mainContent">
          <el-col :sm="{span:4,offset:0}" :md="{span:4,offset:0}" :lg="{span:4,offset:2}" :xl="{span:4,offset:4}"
                  class="hidden-xs-only">
            <el-affix :offset="80">
              <el-tabs tab-position="left" style="width: 100%;" @tab-click="tabClicked($event.props.name)"
                       v-model="selectedTab">
                <el-tab-pane
                    v-for="tab in tabs"
          ...
          >
            <el-card v-if="$store.state.loggedIn===false">
              ...
              <p>
                If you compiled this app from source code and are accessing this page outside the context of an
                extension
                (if you have no clue what this means, you most definitely didn't and feel free to stop reading from
                here),
                and you are sure you have logged in, then you are being blocked by the CORS policy. You need to access
                this
                page inside an extension (that is, within a namespace of <code>chrome-extension://</code>) with host
                permission to gradescope.
                The easiest way is to run <code>yarn build</code> and install the resulting zip file as an unpacked
                extension.
                Alternatively, launch a chromium browser with <code>--disable-web-security</code> flag and try this
                again.
              </p>
            </el-card>
...
export default class OverviewPage extends Vue {
  ...
  updateViewName(name: string) {
    ...
    // @ts-ignore well apparently scrollbar.setScrollTop is not a function...
